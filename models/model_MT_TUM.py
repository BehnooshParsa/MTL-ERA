...
class gcnEdtcnREBA_tanh(nn.Module):
    def __init__(self, hidden=25, kernel_size=8):
        ...
        # self.ln2 = nn.Linear(2048 + num_class, 1024)  # (N,  T, 2075) -> (N, T, 1024)
    ...
    def forward(self, x):
        ...
        x_class = self.tcn(torch.tanh(x.permute(0, 2, 1)))
...
class gcnEdtcnREBA_emb(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 3, 1, 2).unsqueeze(4)
        x = self.adaptpool(self.GCN(x))
        x_class = self.tcn(F.relu(x.permute(0, 2, 1)))
        classification_score = self.l_classification(F.relu(self.ln1(x_class.permute(0, 2, 1))))
        x_reg, _ = self.LSTM1(F.tanh(self.ln2(torch.cat([x, F.softmax(classification_score)], dim=2))))
        regression_output = self.l_regression(x_reg)
...
class gcnEdtcn_class(nn.Module):
    ...
    def forward(self, x):
        ...
        return classification_score, 0  # , regression_output
